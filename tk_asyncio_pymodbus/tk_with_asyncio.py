...
queue_of_commands_for_asyncio_loop: queue.Queue = queue.Queue()
# Queue of tasks generated by asyncio code for handling by Tkinter 
queue_of_tasks_for_tk_loop: queue.Queue = queue.Queue()
...
queue_of_commands_for_asyncio_loop_is_busy = False
...
last_command_for_asyncio_loop_in_queue = ''
...
async def asyncio_service_loop(gui, aio_loop_shutdown_initiated: threading.Event):
    ...
    global queue_of_commands_for_asyncio_loop_is_busy    
    ...
    while not aio_loop_shutdown_initiated.is_set():        
        ...
        if (tk_future_task_scheduled == False):
            try:     
                ...
                command_for_asyncio_loop = queue_of_commands_for_asyncio_loop.get_nowait()
            except queue.Empty:
                pass # Try next time
            else: # Got a Tkinter command from the queue
                queue_of_commands_for_asyncio_loop_is_busy = False

                if (command_for_asyncio_loop != 'Idle'):
                    # Create a future task to handle the Tkinter command asyncronously
                    tk_future_task = asyncio.ensure_future(gui.handle_command_in_asyncio_loop(command_for_asyncio_loop))
                    # Set the flag
                    tk_future_task_scheduled = True
                else:
                    timed_msg('-Asyncio loop: Got ' + command_for_asyncio_loop +' command... Clearing commands for asyncio.')
                    queue_of_commands_for_asyncio_loop.queue.clear()
        ...
        if tk_future_task.done(): 
            try:
                ...
                task_for_tk_loop = tk_future_task.result()
                # Put the task from Asyncio to the queue (don't use the thread-blocking 'put' method)
                queue_of_tasks_for_tk_loop.put_nowait(task_for_tk_loop)
            except queue.Full:
                pass # Try next time
            else:
...
def put_command_for_asyncio_loop_to_queue(command_for_asyncio_loop, mainframe: Frame, fromWhere = 'a button click'):
    poll_interval = 50 # 0.05 s

    global queue_of_commands_for_asyncio_loop_is_busy 
    global last_command_for_asyncio_loop_in_queue
           
    # We should empty the queue_of_commands_for_asyncio_loop and set 'not busy' flag before putting stopping commands
    if (command_for_asyncio_loop == 'StopSensors' or command_for_asyncio_loop == 'Disconnect' or command_for_asyncio_loop == 'Idle'):
        # Unless StopSensors was called from Disconnect button handling
        if (command_for_asyncio_loop == 'Disconnect' and last_command_for_asyncio_loop_in_queue == 'StopSensors' ): # and fromWhere == 'Disconnect handling'
            #timed_msg('-Not clear TK: "' + last_command_for_asyncio_loop_in_queue + '" (before "' + command_for_asyncio_loop + '" which came from ' + fromWhere + ')')
            pass
        else:  
            if (not queue_of_commands_for_asyncio_loop.empty()):
                timed_msg('-Clear TK queue with: "' + last_command_for_asyncio_loop_in_queue + '" (before "' + command_for_asyncio_loop + '" which came from ' + fromWhere + ')')
                queue_of_commands_for_asyncio_loop.queue.clear()
                queue_of_commands_for_asyncio_loop_is_busy = False
                #print(list(queue_of_commands_for_asyncio_loop.queue))
            else:
                #timed_msg('-Not clear TK(empty), Last: "' + last_command_for_asyncio_loop_in_queue + '" (before "' + command_for_asyncio_loop + '" which came from ' + fromWhere + ')')
                pass

    if (not queue_of_commands_for_asyncio_loop.full() and not queue_of_commands_for_asyncio_loop_is_busy and not tk_future_task_scheduled):    
        # Put a command from Tkinter to the queue (but don't wait). Asyncio can't wait for the thread-blocking 'put' method
        queue_of_commands_for_asyncio_loop.put_nowait(command_for_asyncio_loop) 
        timed_msg('-Put TK command: "' + command_for_asyncio_loop + '" to queue from: ' + fromWhere)
        #print(list(queue_of_commands_for_asyncio_loop.queue))
        
        last_command_for_asyncio_loop_in_queue = command_for_asyncio_loop
        queue_of_commands_for_asyncio_loop_is_busy = True
    else:
        mainframe.after(poll_interval, lambda: put_command_for_asyncio_loop_to_queue(command_for_asyncio_loop, mainframe, fromWhere))
...
def tk_service_loop(mainframe: Frame, gui):
    ...
    if (tk_future_task_scheduled == False):
        try:
            ...
            task_for_tk_loop = queue_of_tasks_for_tk_loop.get_nowait()            
        except queue.Empty:
            pass
            ...
            command_for_asyncio_loop = gui.handle_task_in_tk_loop(task_for_tk_loop)
            # Put the Tkinter command to the queue
            put_command_for_asyncio_loop_to_queue(command_for_asyncio_loop, mainframe, 'Asyncio task "' + task_for_tk_loop[0] + '"')
...
def main():    
    ...
    gui = GUI(tk_root, mainframe, put_command_for_asyncio_loop_to_queue, delay_after_chain_of_commands, modbus_timeout, host_ip, port_addr)
