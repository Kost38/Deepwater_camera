...
class GUI:
	...
	# Constructor
	def __init__(self, tk_root, mainframe, put_label_to_queue_for_asyncio_callback, delay_after_chain_of_commands, modbus_timeout, host, port):
		self.tk_root = tk_root
		self.mainframe = mainframe
		self.put_label_to_queue_for_asyncio = put_label_to_queue_for_asyncio_callback
	...
	def initialize_gui(self):
		...
		self.frame_right = Frame(width = 500, height = 550)
...
#		self.frame_left_bottom.pack(side='bottom', anchor='sw', fill='x') # Left Bottom frame.pack()  
	...
	async def handle_command_in_asyncio(self, command_for_asyncio):  
		#timed_msg('-AIO: Got and handling a command "' + command_for_asyncio + '"')

		# Connect and check compass calibration status
		
		if (command_for_asyncio == 'Connect'):
			...
			#timed_msg('-AIO: Command "' + command_for_asyncio + '" awaited')
			return async_modbus_task.result() # ['ConnectCalled'] goes to queue_of_tasks_for_tk
		
		if (command_for_asyncio == 'GetCompassCalibrationStatus'):
			async_modbus_task = asyncio.ensure_future(self.modbus_wrapper.read_cal_status_from_compass())
			await async_modbus_task # Wait for the future
			#timed_msg('-AIO: Command "' + command_for_asyncio + '" awaited')
			return async_modbus_task.result() # 'CompassCalibrationStatusGot' Goes to queue_of_tasks_for_tk
			
		# Compass configuration buttons
		
		if (command_for_asyncio == 'StartCompassCalibration'):
			async_modbus_task = asyncio.ensure_future(self.modbus_wrapper.start_compass_calibration())
			await async_modbus_task # Wait for the future
			#timed_msg('-AIO: Command "' + command_for_asyncio + '" awaited')
			return async_modbus_task.result() # ['CompassCalibrationStarted'] Goes to queue_of_tasks_for_tk
			
		if (command_for_asyncio == 'StopCompassCalibration'):
			async_modbus_task = asyncio.ensure_future(self.modbus_wrapper.stop_compass_calibration())
			await async_modbus_task # Wait for the future
			#timed_msg('-AIO: Command "' + command_for_asyncio + '" awaited')
			return async_modbus_task.result() # ['CompassCalibrationStoped'] Goes to queue_of_tasks_for_tk
			
		if (command_for_asyncio == 'CorrectCompassNorth'):
			async_modbus_task = asyncio.ensure_future(self.modbus_wrapper.correct_compass_north())
			await async_modbus_task # Wait for the future
			#timed_msg('-AIO: Command "' + command_for_asyncio + '" awaited')
			return async_modbus_task.result() # ['CompassNorthCorrected'] Goes to queue_of_tasks_for_tk
			
		if (command_for_asyncio == 'SaveCompassConfiguration'):
			async_modbus_task = asyncio.ensure_future(self.modbus_wrapper.save_compass_configuration())
			await async_modbus_task # Wait for the future
			#timed_msg('-AIO: Command "' + command_for_asyncio + '" awaited')
			return async_modbus_task.result() # ['CompassConfigurationSaved'] Goes to queue_of_tasks_for_tk

		if (command_for_asyncio == 'ResetCompass'):
			async_modbus_task = asyncio.ensure_future(self.modbus_wrapper.reset_compass())
			await async_modbus_task # Wait for the future
			#timed_msg('-AIO: Command "' + command_for_asyncio + '" awaited')
			return async_modbus_task.result() # 'CompassResetCommited' Goes to queue_of_tasks_for_tk
		 
		# Sensors transaction sequence
		 
		if (command_for_asyncio == 'StartSensors'):
			#timed_msg('-AIO: Command "' + command_for_asyncio + '" awaited')
			return ['StartSensorsClicked'] # goes to queue_of_tasks_for_tk 

		if (command_for_asyncio == 'TransactPressure'):
			async_modbus_task = asyncio.ensure_future(self.modbus_wrapper.transact_pressure_sensor())
			await async_modbus_task # Wait for the future
			#timed_msg('-AIO: Command "' + command_for_asyncio + '" awaited')
			return async_modbus_task.result() # ['PressureTransacted', registers] goes to queue_of_tasks_for_tk			
			
		if (command_for_asyncio == 'TransactCompassTempr'):
			async_modbus_task = asyncio.ensure_future(self.modbus_wrapper.read_temperature_from_compass_sensor())
			await async_modbus_task # Wait for the future
			#timed_msg('-AIO: Command "' + command_for_asyncio + '" awaited')
			return async_modbus_task.result() # ['CompassTemprTransacted'] goes to queue_of_tasks_for_tk
			
		if (command_for_asyncio == 'TransactCompassPitchHeading'):
			async_modbus_task = asyncio.ensure_future(self.modbus_wrapper.read_pitch_heading_from_compass_sensor())
			await async_modbus_task # Wait for the future
			#timed_msg('-AIO: Command "' + command_for_asyncio + '" awaited')
			return async_modbus_task.result() # ['CompassPitchHeadTransacted'] goes to queue_of_tasks_for_tk
			
		if (command_for_asyncio == 'WaitForNextIteration'):
			async_modbus_task = asyncio.ensure_future(self.modbus_wrapper.wait_next_iteration())
			await async_modbus_task # Wait for the future
			#timed_msg('-AIO: Command "' + command_for_asyncio + '" awaited')
			return async_modbus_task.result() # ['NextIterationWaited'] goes to queue_of_tasks_for_tk 

		if (command_for_asyncio == 'StopSensors'):
			#timed_msg('-AIO: Command "' + command_for_asyncio + '" awaited')
			return ['StopSensorsClicked'] # Goes to queue_of_tasks_for_tk 
			
		# Light buttons

		if (command_for_asyncio == 'Light1 ON'):
			async_modbus_task = asyncio.ensure_future(self.modbus_wrapper.turn_light1_on())
			await async_modbus_task # Wait for the future
			#timed_msg('-AIO: Command "' + command_for_asyncio + '" awaited')
			return async_modbus_task.result() # ['Light1TurnedON'] goes to queue_of_tasks_for_tk 
		
		if (command_for_asyncio == 'Light1 OFF'):
			async_modbus_task = asyncio.ensure_future(self.modbus_wrapper.turn_light1_off())
			await async_modbus_task # Wait for the future
			#timed_msg('-AIO: Command "' + command_for_asyncio + '" awaited')
			return async_modbus_task.result() # ['Light1TurnedOFF'] goes to queue_of_tasks_for_tk 

		if (command_for_asyncio == 'Light2 ON'):
			async_modbus_task = asyncio.ensure_future(self.modbus_wrapper.turn_light2_on())
			await async_modbus_task # Wait for the future
			#timed_msg('-AIO: Command "' + command_for_asyncio + '" awaited')
			return async_modbus_task.result() # ['Light2TurnedON'] goes to queue_of_tasks_for_tk 
		
		if (command_for_asyncio == 'Light2 OFF'):
			async_modbus_task = asyncio.ensure_future(self.modbus_wrapper.turn_light2_off())
			await async_modbus_task # Wait for the future
			#timed_msg('-AIO: Command "' + command_for_asyncio + '" awaited')
			return async_modbus_task.result() # ['Light2TurnedOFF'] goes to queue_of_tasks_for_tk 
			
		if (command_for_asyncio == 'Light3 ON'):
			async_modbus_task = asyncio.ensure_future(self.modbus_wrapper.turn_light3_on())
			await async_modbus_task # Wait for the future
			#timed_msg('-AIO: Command "' + command_for_asyncio + '" awaited')
			return async_modbus_task.result() # ['Light3TurnedON'] goes to queue_of_tasks_for_tk 
		
		if (command_for_asyncio == 'Light3 OFF'):
			async_modbus_task = asyncio.ensure_future(self.modbus_wrapper.turn_light3_off())
			await async_modbus_task # Wait for the future
			#timed_msg('-AIO: Command "' + command_for_asyncio + '" awaited')
			return async_modbus_task.result() # ['Light3TurnedOFF'] goes to queue_of_tasks_for_tk
			
		if (command_for_asyncio == 'Light4 ON'):
			async_modbus_task = asyncio.ensure_future(self.modbus_wrapper.turn_light4_on())
			await async_modbus_task # Wait for the future
			#timed_msg('-AIO: Command "' + command_for_asyncio + '" awaited')
			return async_modbus_task.result() # ['Light4TurnedON'] goes to queue_of_tasks_for_tk 
		
		if (command_for_asyncio == 'Light4 OFF'):
			async_modbus_task = asyncio.ensure_future(self.modbus_wrapper.turn_light4_off())
			await async_modbus_task # Wait for the future
			#timed_msg('-AIO: Command "' + command_for_asyncio + '" awaited')
			return async_modbus_task.result() # ['Light4TurnedOFF'] goes to queue_of_tasks_for_tk 

		# Disconnect

		if (command_for_asyncio == 'Disconnect'):
			async_modbus_task = asyncio.ensure_future(self.modbus_wrapper.close_client())
			await async_modbus_task # Wait for the future
			#timed_msg('-AIO: Command "' + command_for_asyncio + '" awaited')
			return async_modbus_task.result() # 'Disconnected' goes to queue_of_tasks_for_tk

		return [None] # Goes to queue_of_tasks_for_tk if command_for_asyncio is unknown


	def handle_task_in_tk(self, task_for_tk):
		#timed_msg('-TK: Got and handling a task: "' + task_for_tk[0] + '"')
		
		# Connect button
		
		# Handle task for TK whisch is reply to command for Asyncio 'Connect'
		if (task_for_tk[0] == 'ConnectCalled'):
			if (self.modbus_wrapper.connection_status == 'Connected'):
				self.switch_on_connect()
				return 'GetCompassCalibrationStatus' # Goes to queue_of_labels_for_asyncio
			else:
				self.switch_on_disconnect()
				timed_msg('handle_task_in_tk(ConnectCalled): Modbus connection status is not connected after asyncio "Connect" command')
				return None # Goes to queue_of_labels_for_asyncio
				
		# Handle task for TK whisch is reply to command for Asyncio 'GetCompassCalibrationStatus'
		if (task_for_tk[0] == 'CompassCalibrationStatusGot'):
			self.CalStatus_val.set(str(task_for_tk[1])[:4])
			self.modbus_log_text.insert('end', 'CalStatus: ' + str(task_for_tk[1]) + '\n')
			self.modbus_log_text.yview('end')
			return None # Goes to queue_of_labels_for_asyncio

		# Compass configuration buttons

		# Handle task for TK whisch is reply to command for Asyncio 'StartCompassCalibration'
		if (task_for_tk[0] == 'CompassCalibrationStarted'):
			self.switch_on_start_compass_calibration()
			return 'GetCompassCalibrationStatus' # Goes to queue_of_labels_for_asyncio
			
		# Handle task for TK whisch is reply to command for Asyncio 'StopCompassCalibration'
		if (task_for_tk[0] == 'CompassCalibrationStoped'):
			self.switch_on_stop_compass_calibration()
			return 'GetCompassCalibrationStatus' # Goes to queue_of_labels_for_asyncio   
			
		# Handle task for TK whisch is reply to command for Asyncio 'CorrectCompassNorth'
		if (task_for_tk[0] == 'CompassNorthCorrected'):
			return 'GetCompassCalibrationStatus' # Goes to queue_of_labels_for_asyncio
			
		# Handle task for TK whisch is reply to command for Asyncio 'SaveCompassConfiguration'
		if (task_for_tk[0] == 'CompassConfigurationSaved'):
			return 'GetCompassCalibrationStatus' # Goes to queue_of_labels_for_asyncio

		# Handle task for TK whisch is reply to command for Asyncio 'ResetCompass'
		if (task_for_tk[0] == 'CompassResetCommited'):
			return 'GetCompassCalibrationStatus' # Goes to queue_of_labels_for_asyncio
		
		# Sensors transaction sequence

		# Handle task for TK whisch is reply to command for Asyncio 'StartSensors'
		if (task_for_tk[0] == 'StartSensorsClicked'):
			self.sensors_chain_started = True
			self.switch_on_start_sensors()
			return 'TransactPressure' # Goes to queue_of_labels_for_asyncio

		# Handle task for TK whisch is reply to command for Asyncio 'TransactPressure'
		if (task_for_tk[0] == 'PressureTransacted'):
			self.pressure_sensor_val.set(str(f"{format(task_for_tk[1], '.1f')}"))
		...
		# Handle task for TK whisch is reply to command for Asyncio 'TransactCompassTempr'
		if (task_for_tk[0] == 'CompassTemprTransacted'):
			self.Tempr_val.set(str(task_for_tk[1])[:4])
			self.modbus_log_text.insert('end', 'Temperature: ' + str(task_for_tk[1]) + '\n')
			self.modbus_log_text.yview('end')
			return 'TransactCompassPitchHeading' # Goes to queue_of_labels_for_asyncio

		# Handle task for TK whisch is reply to command for Asyncio 'TransactCompassPitchHeading'
		if (task_for_tk[0] == 'CompassPitchHeadTransacted'):
			self.Pitch_val.set(str(task_for_tk[1])[:4])
			self.Heading_val.set(str(task_for_tk[2])[:4])
			self.modbus_log_text.insert('end', 'Pitch: ' + str(task_for_tk[1]) + '\n') 
			self.modbus_log_text.insert('end', 'Heading: ' + str(task_for_tk[2]) + '\n')
			self.modbus_log_text.yview('end')
			return 'WaitForNextIteration' # Goes to queue_of_labels_for_asyncio

		# Handle task for TK whisch is reply to command for Asyncio 'WaitForNextIteration'
		if (task_for_tk[0] == 'NextIterationWaited'):
			if self.sensors_chain_started == True:
				return 'TransactPressure' # Goes to queue_of_labels_for_asyncio
			else:
				return None # Goes to queue_of_labels_for_asyncio

		# Handle task for TK whisch is reply to command for Asyncio 'StopSensors'
		if (task_for_tk[0] == 'StopSensorsClicked'):
			self.sensors_chain_started = False
		...
		# Handle task for TK whisch is reply to command for Asyncio 'Light1 ON'
		if (task_for_tk[0] == 'Light1TurnedON'):
			self.switch_on_light1on()
			return None # Goes to queue_of_labels_for_asyncio  

		# Handle task for TK whisch is reply to command for Asyncio 'Light1 OFF'
		if (task_for_tk[0] == 'Light1TurnedOFF'):
			self.switch_on_light1off()
			return None # Goes to queue_of_labels_for_asyncio

		# Handle task for TK whisch is reply to command for Asyncio 'Light2 ON'
		if (task_for_tk[0] == 'Light2TurnedON'):
			self.switch_on_light2on()
			return None # Goes to queue_of_labels_for_asyncio  

		# Handle task for TK whisch is reply to command for Asyncio 'Light2 OFF'
		if (task_for_tk[0] == 'Light2TurnedOFF'):
			self.switch_on_light2off()
			return None # Goes to queue_of_labels_for_asyncio

		# Handle task for TK whisch is reply to command for Asyncio 'Light3 ON'
		if (task_for_tk[0] == 'Light3TurnedON'):
			self.switch_on_light3on()
			return None # Goes to queue_of_labels_for_asyncio  

		# Handle task for TK whisch is reply to command for Asyncio 'Light3 OFF'
		if (task_for_tk[0] == 'Light3TurnedOFF'):
			self.switch_on_light3off()
			return None # Goes to queue_of_labels_for_asyncio

		# Handle task for TK whisch is reply to command for Asyncio 'Light4 ON'
		if (task_for_tk[0] == 'Light4TurnedON'):
			self.switch_on_light4on()
			return None # Goes to queue_of_labels_for_asyncio  

		# Handle task for TK whisch is reply to command for Asyncio 'Light4 OFF'
		if (task_for_tk[0] == 'Light4TurnedOFF'):
			self.switch_on_light4off()
			return None # Goes to queue_of_labels_for_asyncio

		# Disconnect

		# Handle task for TK whisch is reply to command for Asyncio 'Disconnect'
		if (task_for_tk[0] == 'Disconnected'):
			self.switch_on_disconnect()
			#timed_msg('handle_task_in_tk: Disconnected')
			return None # Goes to queue_of_labels_for_asyncio  
			
		timed_msg('handle_task_in_tk(): task is unknown')
		return None # Goes to queue_of_labels_for_asyncio if task_for_tk is unknown
	...
	def on_connect(self):
		if self.connect_button.config('relief')[-1] == 'raised':
			self.put_label_to_queue_for_asyncio('Connect', self.mainframe)			
		else:
			# Stop sensors if needed
			if self.start_sensors_button.config('relief')[-1] == 'sunken':				
				self.put_label_to_queue_for_asyncio('StopSensors', self.mainframe) 
				self.put_label_to_queue_for_asyncio('Disconnect', self.mainframe) 
			else:
				self.put_label_to_queue_for_asyncio('Disconnect', self.mainframe)
	...
	def on_start_sensors(self): 
		if self.start_sensors_button.config('relief')[-1] == 'raised':
			self.put_label_to_queue_for_asyncio('StartSensors', self.mainframe)
		else:
			self.put_label_to_queue_for_asyncio('StopSensors', self.mainframe) 
	...
	def on_calibrate_compass(self): 
		if self.calibrate_compass_button.config('relief')[-1] == 'raised':
			self.put_label_to_queue_for_asyncio('StartCompassCalibration', self.mainframe)
		else:
			self.put_label_to_queue_for_asyncio('StopCompassCalibration', self.mainframe) 
	...
	def on_correct_compass_north(self): 
		self.put_label_to_queue_for_asyncio('CorrectCompassNorth', self.mainframe)
		 
	# Handle "Save Compass Configuration" button click
	def on_save_compass_configuration(self): 
		self.put_label_to_queue_for_asyncio('SaveCompassConfiguration', self.mainframe)  

	# Handle "Reset compass" button click
	def on_reset_compass(self): 
		self.put_label_to_queue_for_asyncio('ResetCompass', self.mainframe)

	# Handle "Reset Get CalStatus" button click
	def on_get_compass_calibration_status(self):
		self.put_label_to_queue_for_asyncio('GetCompassCalibrationStatus', self.mainframe)
...
#		self.frame_light.pack(side='right', anchor='se') # Light buttons frame.pack()
	...
	def on_light1(self):
		if self.light1_button.config('relief')[-1] == 'sunken':
			self.put_label_to_queue_for_asyncio('Light1 OFF', self.mainframe)
		else:
			self.put_label_to_queue_for_asyncio('Light1 ON', self.mainframe) 
	...
	def on_light2(self):
		if self.light2_button.config('relief')[-1] == 'sunken':
			self.put_label_to_queue_for_asyncio('Light2 OFF', self.mainframe)
		else:
			self.put_label_to_queue_for_asyncio('Light2 ON', self.mainframe)
	...
	def on_light3(self):
		if self.light3_button.config('relief')[-1] == 'sunken':
			self.put_label_to_queue_for_asyncio('Light3 OFF', self.mainframe)
		else:
			self.put_label_to_queue_for_asyncio('Light3 ON', self.mainframe)
	...
	def on_light4(self):
		if self.light4_button.config('relief')[-1] == 'sunken':
			self.put_label_to_queue_for_asyncio('Light4 OFF', self.mainframe)
		else:
			self.put_label_to_queue_for_asyncio('Light4 ON', self.mainframe) 
